...
filterByDirSize :: Int -> Node -> [Int]
filterByDirSize min_size node@(Dir (_, nodes) dsize)=
    let rest = concat $ map (filterByDirSize min_size) $ filter isDir nodes
    in if dsize >= min_size then
        dsize : rest
    else
        rest

main :: IO ()
main = interact $ show . minimum . (\root@(Dir _ total_used) -> filterByDirSize (total_used - 40000000) root) . setDirSizes . newDir . fst . (customFoldL executeCommand (("/", []), [])) . parseCommands . lines
